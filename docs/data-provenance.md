...
| `rerun_2026_04_26_pwt_final_maxinterval/` | same | same | 2026-04-26 | `false` | (same as baseline) | Back-to-back rerun of `pwt_maxinterval` — controlled session for between-session noise estimation. Originally produced by the now-deleted `run_pwt_final_ab.sh` (a 2-call wrapper); reproduce by running the orchestrator twice back-to-back with appropriate `--docker-flags` and `--tag`. |
| `rerun_2026_04_26_pwt_final_combined/` | same | same | 2026-04-26 | `false` | (same as `pwt_rtflags`) | Back-to-back rerun of `pwt_combined` — controlled session pair with the row above. Reproduce as `pwt_rtflags` immediately after `pwt_final_maxinterval`. |
| `rerun_2026_04_26_paper/` | same | same | 2026-04-26 | `false` | (none, only the `-e HOME=/tmp` defaults `docker_wrap` applies) | 5 Vicon × stereo + mono serial — RPi5 paper-repro for x86-vs-RPi5 cross-platform comparison. Reproduce: `bash scripts/run_full_benchmark.sh -m serial -c both -r 1 -s V1_01_easy,V1_02_medium,V1_03_difficult,V2_01_easy,V2_02_medium --docker openvins-humble:latest --tag rerun_2026_04_26_paper` |
